...
public class SuperSorterProgressSnapshotTest
{
  ...
  public void testSnapshotWhenTotalMergingLevelsUnset()
  {
    ...
    Assertions.assertNull(snapshot.getProgressDigest());
    Assertions.assertEquals(-1, snapshot.getTotalMergingLevels());
    Assertions.assertEquals(LEVEL_TO_TOTAL_MERGERS_COMMON_MAP, snapshot.getLevelToTotalBatches());
    Assertions.assertEquals(LEVEL_TO_MERGED_BATCHES_COMMON_MAP, snapshot.getLevelToMergedBatches());
    Assertions.assertEquals(false, snapshot.isTriviallyComplete());
  }
  ...
  public void testSnapshotWhenTotalMergingLevelsSetButOutputPartitionsCountUnknown()
  {
    ...
    Assertions.assertNotNull(snapshot.getProgressDigest());

    // 10/16 + 2/8 + 3/4 + 1/2 + 0 (ultimate mergers shouldn't be taken into account if not set explicitly)
    Assertions.assertEquals(0.425D, snapshot.getProgressDigest(), 1e-6);
    Assertions.assertEquals(TOTAL_LEVELS, snapshot.getTotalMergingLevels());
    ...
    Assertions.assertEquals(levelToTotalMergersModifiedMap, snapshot.getLevelToTotalBatches());
    Assertions.assertEquals(LEVEL_TO_MERGED_BATCHES_COMMON_MAP, snapshot.getLevelToMergedBatches());
    Assertions.assertEquals(false, snapshot.isTriviallyComplete());
  }
  ...
  public void testSnapshotWhenTotalMergingLevelsAndOutputPartitionsCountKnown()
  {
    ...
    Assertions.assertEquals(0.521D, snapshot.getProgressDigest(), 1e-6);
    Assertions.assertEquals(TOTAL_LEVELS, snapshot.getTotalMergingLevels());
    Map<Integer, Long> levelToTotalMergersModifiedMap = new HashMap<>(LEVEL_TO_TOTAL_MERGERS_COMMON_MAP);
  ...
  public void testSnapshotWhenTrackerIsTriviallyComplete()
  {
    ...
    SuperSorterProgressSnapshot snapshot = superSorterProgressTracker.snapshot();
    Assertions.assertNotNull(snapshot.getProgressDigest());
    Assertions.assertEquals(1.0, snapshot.getProgressDigest(), 0.0);
    Assertions.assertEquals(true, snapshot.isTriviallyComplete());
  }
  ...
  public void testSerde() throws Exception
  {
    ...
    Assertions.assertEquals(snapshot, jsonMapper.readValue(jsonString, SuperSorterProgressSnapshot.class));
  }
