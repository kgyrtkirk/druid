...
public class DruidConnectionStateListenerTest
{
  ...
  public void test_doMonitor_notConnected()
  {
    listener.stateChanged(null, ConnectionState.SUSPENDED);
    listener.doMonitor(emitter);
    Assertions.assertEquals(2, emitter.getEvents().size()); // 2 because stateChanged emitted an alert
  ...
  {
    listener.stateChanged(null, ConnectionState.SUSPENDED);
    Assertions.assertEquals(1, emitter.getEvents().size()); // the first stateChanged emits an alert

    listener.stateChanged(null, ConnectionState.RECONNECTED);
    Assertions.assertEquals(2, emitter.getEvents().size()); // the second stateChanged emits a metric
