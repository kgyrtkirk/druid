...
public class Quidem1Test {
  ...
  protected void checkRun(String path) throws Exception {
    ...
    if (f.isAbsolute()) {
      ...
      URL ff = getClass().getProtectionDomain().getCodeSource().getLocation();
      final URL inUrl = Quidem1Test.class.getResource("/" + n2u(path));
      inFile = Sources.of(requireNonNull(inUrl, "inUrl")).file();
      outFile = replaceDir(inFile, "quidem", "quidem" );
    }
